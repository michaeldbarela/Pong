...
//////////////////////////////////////////
    ...
    wire           aiso_reset, EN_BW;
    wire    [9:0]  pixel_x, pixel_y;
    wire    [11:0] bottom_wall_rgb;
    integer        i, j, errorCount;
    
//////////////////////////////////////////
//         Device Under Test
//////////////////////////////////////////
    pong_bottom_wall dut( .select( select ),
                          .SW_red( SW_red ),
                          .SW_green( SW_green ),
                          .SW_blue( SW_blue ),
                          .pixel_x( pixel_x ),
                          .pixel_y( pixel_y ),
                          .EN_BW( EN_BW ),
                          .bottom_wall_rgb( bottom_wall_rgb ) );
...
//////////////////////////////////////////                            
    ...
    initial begin
        ...
        #10 reset = 1;
        if(errorCount == 0)
            $display("NO ERRORS FOUND IN PONG BOTTOM WALL MODULE.");
        #10 $stop;
    end
    ...
    task count; begin
        ...
        for( i = 0; i < 525; i = i + 1 ) begin
            for( j = 0; j < 800; j = j + 1 ) begin
                ...
                if( pixel_y >= dut.BWT && pixel_y <= dut.BWB ) begin
                    if( pixel_x >= dut.BWL && pixel_x <= dut.BWR ) begin
                        if( bottom_wall_rgb != ~{SW_red, SW_green, SW_blue} ) begin
                            $display("ERROR: INCORRECT COLOR DISPLAYED FOR BOTTOM WALL AT PIXEL X = %d and PIXEL Y = %d", pixel_x, pixel_y);
                            errorCount = errorCount + 1;
                        end
                        if( !EN_BW ) begin
                            $display("ERROR: ENABLE BIT INCORRECT FOR BOTTOM WALL AT PIXEL X = %d and PIXEL Y = %d", pixel_x, pixel_y);
                            errorCount = errorCount + 1;
                        end
                    end
                end
